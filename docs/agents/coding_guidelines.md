...
`app/services/*.ts` (`csvParser.ts`, `columnStats.ts`, `columnFilters.ts`, `viewerSession.ts`, `exportData.ts`, `exportXlsx.ts`, `formatFile.ts`) contain zero Vue imports — plain functions/types only, runnable inside a Web Worker or a plain Vitest test. `viewerSession.ts:1-9` states the convention explicitly ("Funções puras, sem import de Vue/composables ... seguindo a convenção de `app/services/`"). Rationale: `csvParser.worker.ts` reuses `runParseRequest` from `csvParser.ts` directly (`/// <reference lib="webworker" />` + `self.addEventListener`), and each service ships its own `*.spec.ts` with no component mount required. No enforcement tool; verified by import inspection of `app/services/csvParser.ts`, `app/services/columnStats.ts`, and `app/services/columnFilters.ts`.

### 2. Module-scope singleton state in composables
Shared state lives as module-level `ref`s outside the composable function, so multiple call sites share one source of truth without a store library. Observed in `useCurrentDataset.ts:36-37` (`const dataset = ref<Dataset | null>(null)`), `useOpenFile.ts:55-57` (`error`, `isOpening`, `progress`), `useTheme.ts:21-22` (`theme`, `initialized`), and `useCellEditing.ts:50-53` (`editingCell`, `validationError`, `undoStack`, `redoStack` — reset via a module-level `watch` on the dataset's `id`, `useCellEditing.ts:61-70`). Composables expose `readonly()` wrappers over the refs (`useCurrentDataset.ts:55-56`, `useOpenFile.ts:185-187`, `useCellEditing.ts:183-184`) so consumers cannot mutate state directly.

### 3. Dependency injection via options objects, defaulted for production
Composables that touch IO (Worker, IndexedDB, router) accept an options object where every dependency has a production default, enabling tests to substitute fakes. Observed in `useCsvParser(options: UseCsvParserOptions = {})` (`createWorker` defaults to `defaultCreateWorker`, `useCsvParser.ts:95-96`), `useOpenFile(options: UseOpenFileOptions = {})` (`parser`, `filesStore`, `currentDataset`, `navigate` all defaulted, `useOpenFile.ts:65-70`), `useSaveVersion(options: UseSaveVersionOptions = {})` (`currentDataset`/`filesStore` default to `useCurrentDataset()`/`useFilesStore()`, `useSaveVersion.ts:28-30`), and `useViewerSession(viewer, meta, options: UseViewerSessionOptions = {})` (`sessionStore` defaults to `useSessionStore()`, `debounceMs` defaults to `300`, `useViewerSession.ts:66-67`). Rationale: `test/useOpenFile.spec.ts`, `test/useCsvParser.spec.ts`, `test/useSaveVersion.spec.ts`, and `test/useViewerSession.spec.ts` inject deterministic fakes instead of a real Worker/IndexedDB/router.
