...
Legenda: um `FilterPanel` novo (UI-01) coleta filtros por coluna num rascunho local e só os entrega ao `useViewer` de uma vez, no "Filtrar" (`applyFilters`, RF-05 v1.2); um `FilterChips` novo exibe os filtros já aplicados como badges e remove um deles de imediato (`removeFilter`, sem passar pelo drawer). Um módulo de predicado puro novo (`columnFilters`, CT-01) avalia cada linha por tipo/operador (RF-01, RF-02) e `useViewer` compõe busca + filtros por E (RF-03, RF-04, CT-02) atualizando `filteredRows`. A `ViewerToolbar` ganha o badge de contagem de filtros aplicados (UI-02, alterado); `noResults` passa a refletir a combinação vazia (RF-06). Os nós `useViewer`/`ViewerTable`/`ViewerToolbar`/`SearchInput` são verificados no código; `FilterPanel`, `FilterChips` e `columnFilters` são novos.
...
- RF-05 [Event-Driven] — O editor de filtros (drawer "Filtros avançados") mantém um **rascunho local**, semeado a partir dos filtros já aplicados sempre que abre; criar/editar/remover um card no drawer altera **só o rascunho** e NÃO DEVE refletir na tabela. O resultado só passa a refletir o rascunho quando o usuário aciona o botão **"Filtrar"**, que substitui de uma vez o conjunto de filtros aplicados e fecha o drawer; fechar o drawer por outro meio (X, backdrop, Esc) DEVE descartar o rascunho sem aplicar. Remover um filtro **já aplicado** pelo "×" do badge acima da tabela (fora do drawer) É a exceção: essa remoção DEVE refletir **em tempo real**, sem passar pelo "Filtrar". O contador de linhas exibido (`visibleRowCount`, `useViewer.ts:106`) DEVE atualizar para a nova contagem em qualquer um dos dois casos, sem materializar todas as linhas no DOM.
  - Um filtro aplicado que ainda **não** tenha um valor válido (operador exige valor mas ele está vazio) é **inerte**: é ignorado (equivale a não restringir nenhuma linha) até receber um valor válido.
  - AC: editar coluna/operador/valor de um card no drawer, ou usar "Adicionar filtro"/"Limpar" nele, não muda o contador de linhas nem os badges enquanto o drawer permanece aberto; clicar em "Filtrar" aplica o rascunho inteiro de uma vez (contador e badges atualizam, drawer fecha); fechar o drawer sem clicar em "Filtrar" descarta as edições — reabri-lo mostra de novo os filtros aplicados antes; clicar no "×" de um badge acima da tabela remove aquele filtro aplicado imediatamente, sem exigir abrir o drawer; um filtro sem valor válido não altera o conjunto de linhas; a virtualização (`@tanstack/vue-virtual`, `ViewerTable.vue:59`) continua montando apenas as linhas visíveis mais overscan.
  - Resolução do conflito com o init chain: a Screen 3 do design (`README.md:90`) listava uma ação "Aplicar"; a v1.1 desta spec resolvera por reflexo puramente reativo sem esse botão. Feedback direto do desenvolvedor reverteu essa decisão (v1.2): o drawer volta a ter um botão explícito de aplicar ("Filtrar"), fiel à Screen 3, e os filtros aplicados aparecem como badges com remoção instantânea acima da tabela (fora do drawer) — ver Resolved Clarifications v1.2.
...
- UI-01 [Ubiquitous] — O Viewer DEVE apresentar (a) uma **barra de badges dos filtros aplicados** acima da tabela (`FilterChips.vue`), fora do cabeçalho da coluna, e (b) um **drawer de edição dedicado** ("Filtros avançados", `FilterPanel.vue`) com um botão **"Adicionar filtro"**, aberto pelo controle "Filtros" da toolbar. Os controles de filtro NÃO DEVEM ser embutidos no cabeçalho da coluna (`ViewerTable.vue` `<th>`).
  - AC: existe uma barra acima da tabela com um badge por filtro **aplicado** (rótulo legível `<coluna> <operador> <valor>`, exibindo o nome da coluna independentemente de estar visível ou oculta); o cabeçalho da coluna (`viewer-table__th`) permanece sem controles de filtro; o drawer "Filtros avançados" tem um botão "Adicionar filtro" que cria um card no rascunho para escolher coluna (o seletor lista **todas** as colunas, inclusive ocultas), operador (conforme RF-01) e valor.

- UI-02 [Event-Driven] — Ao clicar em **"Filtrar"** no drawer, a tabela virtualizada e o contador de linhas DEVEM atualizar de uma vez (RF-05), e a `ViewerToolbar` DEVE indicar o número de filtros aplicados por um badge de contagem no controle "Filtros" (previsto no design, `README.md:78`). Remover um badge aplicado pelo "×" (fora do drawer) DEVE atualizar tabela/contador/badge de contagem imediatamente, sem exigir "Filtrar".
  - AC: clicar em "Filtrar" com N cards no rascunho atualiza o badge de contagem para N e a tabela para o resultado combinado; remover um badge aplicado pelo "×" decrementa o badge de contagem e reamplia a tabela na hora; com zero filtros aplicados o badge não mostra contagem (ou fica em zero) e nenhum badge é exibido acima da tabela.
...
- **UI** (v1.2): dois componentes novos entre a `ViewerToolbar` e o corpo do Viewer (`viewer.vue`) — `FilterChips.vue` (badges dos filtros **aplicados**, reaproveitando os tokens de `Badge.vue`, remoção imediata pelo "×") e `FilterPanel.vue` (drawer lateral "Filtros avançados", fiel à Screen 3, com rascunho local aplicado só no "Filtrar"). Rótulos/helpers de operador compartilhados por ambos vivem em `app/services/filterLabels.ts`. O botão "Filtros" com badge na toolbar espelha a contagem de filtros **aplicados** (`activeFilterCount`, sem contar o rascunho em edição).
...
| RF-05 | Drawer edita um rascunho; só "Filtrar" aplica; remover badge aplicado é imediato; contador atualiza nos dois casos | Sim (component) |
...
| UI-01 | Badges dos filtros aplicados acima da tabela + drawer dedicado com "Adicionar filtro", fora do cabeçalho | Sim (component) |
| UI-02 | Badge de contagem de filtros aplicados na toolbar; "Filtrar" aplica de uma vez, remoção de badge é imediata | Sim (component) |
...
## Resolved Clarifications (v1.2)
12. RF-05/UI-01/UI-02 — **Reversão de #6 (v1.1)**, por feedback direto do desenvolvedor: o drawer volta a ter um botão explícito **"Filtrar"**, fiel à Screen 3 — criar/editar/remover filtros no drawer só altera um **rascunho local**, aplicado de uma vez ao clicar em "Filtrar" (que também fecha o drawer); fechar por outro meio descarta o rascunho. Os filtros **já aplicados** aparecem como badges numa barra dedicada acima da tabela (`FilterChips.vue`), cada um com um "×" que remove aquele filtro **imediatamente**, sem passar pelo "Filtrar" — preservando para essa ação específica o reflexo em tempo real que a v1.1 exigia para tudo. ✔
