...
## Open Questions
- RF-03 não especifica se uma linha deve ser destacada quando o valor duplicado existe apenas numa
  coluna **oculta** (não exibida via seletor de colunas) — diferente de "filtro/busca ativos"
  (esse caso o SPEC já resolve: persiste). A decisão adotada no plano (T01/T02) é escanear **todas**
  as colunas do dataset, inclusive ocultas, por consistência com a invariante de
  `docs/agents/domain_rules.md` ("estatísticas independem de visibilidade de coluna"). Impacto se
  incorreto: uma linha pode aparecer destacada sem nenhuma célula visível duplicada, confundindo o
  usuário — se o comportamento desejado for restringir a colunas visíveis, T01/T02 precisam
  receber `visibleColumns` em vez do dataset completo.
- RF-03 (SPEC) e as sugestões FLEXIBLE não atribuem um token de cor explícito ao destaque de linha
  duplicada (apenas aos quatro destaques de célula: vazio/duplicado/negativo/data inválida). O
  plano assume reaproveitar a família `--accent`/`--accent-soft` (mesma cor do badge de duplicado,
  RF-02) num tom/camada distinto do já usado por `csv-cell--selected`, mas isso é uma decisão de
  implementação (T06) sem validação visual prévia contra
  `.spec/init/design/screen-5-highlights.png` (a referência mostra os 4 tipos de célula, não o
  destaque de linha). Recomenda-se confirmar visualmente antes de fechar T06.
